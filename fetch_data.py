...
def generate_daily_summary(df_today):
    ...
    try:
        ...
        for i, row in df_summary.iterrows():
            ...
            df_summary.at[i, "涨停环比"] = f"{row['涨停数'] - l_up:+}"
            df_summary.at[i, "跌停环比"] = f"{row['跌停数'] - l_down:+}"
    except:
        df_summary["涨停环比"] = 0
        df_summary["跌停环比"] = 0
...
def update_limit_streak(df_today):
    ...
    for _, s in today_up.iterrows():
        prev = streak[streak["代码"] == s["代码"]]["连板天数"].max()
        days = int(prev) + 1 if pd.notna(prev) else 1
        new_rows.append({
            "日期": today, "代码": s["代码"], "名称": s["名称"], "行业": s["行业"], "连板天数": days
        })
